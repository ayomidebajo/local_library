...
exports.bookinstance_delete_get = function (req, res, next) {
  BookInstance.findById(req.params.id)
    ...
    .exec(function (err, results) {
      console.log(results, "logging books inst");
      if (err) {
        return next(err);
      }
...
exports.bookinstance_update_post = [
  (req, res, next) => {
    if (!(req.body.book instanceof Array)) {
      ...
      else req.body.book = new Array(req.body.book);
    }
    next();
  },
  ...
  body("imprint", "Imprint must not be empty.")
    ...
    .isLength({ min: 1 })
    .escape(),
  body("due_back", "Due date must not be empty.")
    .optional({
      checkFalsy: true,
    })
    .isISO8601()
    .toDate(),
  body("status", "Status must not be empty").optional({
    checkFalsy: false,
  }),
